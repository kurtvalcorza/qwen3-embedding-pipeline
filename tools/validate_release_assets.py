"""Static release-asset validation for the Qwen3 text embedding DIMER pipeline.
...
EXPECTED_OUTPUTS = (
    ...
    'outputs/qwen3_embedding_vectors.csv',
)
# Profile-specific code the notebook must exercise through the repository public API.
CODE_MARKERS = (
    'from qwen3_embedding_pipeline import DEFAULT_QUERY_INSTRUCTION, EMBEDDING_DIM, '
    'MAX_BATCH, MAX_TEXT_CHARS, MAX_TEXT_TOKENS',
    'from qwen3_embedding_pipeline import MODEL_ID, MODEL_KEY, MODEL_REVISION, '
    'Qwen3EmbeddingPipeline, cosine_similarity, stage_missing_files, verify_snapshot',
    ...
    "print({'ceilings': {'MAX_BATCH': MAX_BATCH, 'MAX_TEXT_TOKENS': MAX_TEXT_TOKENS, "
    "'MAX_TEXT_CHARS': MAX_TEXT_CHARS, 'EMBEDDING_DIM': EMBEDDING_DIM}})",
...
MARKDOWN_MARKERS = (
    ...
    '**Precision differs by device:**',
    'The unit of embedding is **one vector per text**',
    'Missing data has no meaning here: empty strings are rejected, not embedded.',
    '**No intrinsic metric exists** for an embedding',
    'as a **qualitative check** that the contract works',
    'reranking (see the sibling Qwen3 reranker pipeline), text generation or chat, '
    'classification, clustering quality, retrieval evaluation',
)
