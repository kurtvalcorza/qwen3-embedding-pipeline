...
WORKSHOP_NOTEBOOK_NAME = "DIMER_Qwen3_Semantic_Search_Reranking_Workshop.ipynb"
...
BYOD_GATES = ("USE_BYOD",)
EXPECTED_OUTPUTS = (
    "outputs/qwen3_embedding_train.csv",
    "outputs/qwen3_embedding_input_manifest.json",
    "outputs/qwen3_embedding_evaluation_report.json",
    "outputs/qwen3_embedding_retrieval.csv",
    "outputs/qwen3_embedding_adapter",
    "outputs/qwen3_embedding_result.json",
)
CODE_MARKERS = (
    # Stage 4: pinned corpus, validation, document set, disjoint split, CSV, refusal probes
    "USE_BYOD = False",
    ...
    # Stage 5: the embedding contract with its manifest, rejection probe and sanity checks
    ...
    "'ceilings': {'MAX_BATCH': MAX_BATCH, 'MAX_TEXT_TOKENS': MAX_TEXT_TOKENS, 'MAX_TEXT_CHARS': MAX_TEXT_CHARS, 'EMBEDDING_DIM': EMBEDDING_DIM, 'MAX_TRAIN_TOKENS': MAX_TRAIN_TOKENS, 'KINDS': KINDS}",
    "'unit_norm': bool(np.allclose(np.linalg.norm(doc_vectors, axis=1), 1.0, atol=1e-4))",
    ...
    # Stage 6: the random floor, the lexical baseline and the frozen retrieval metrics
    ...
    # Stage 7: bounded contrastive fine-tuning with explicit hyperparameters
    ...
    # Stage 9: retrieval before/after, batch report, artifact, reload parity, provenance
...
MARKDOWN_MARKERS = (
    "**Capability:** text embeddings (1024-d, last-token pooled, L2-normalised, instruction-aware queries)",
    "and bounded contrastive fine-tuning of the last decoder layers on query–positive pairs, measured by held-out retrieval recall@k and MRR",
    "**Embeddings are representations, not predictions:**",
    "**adaptation measured by retrieval**",
    "**random floor**",
    "**lexical baseline**",
    "**cosine is a similarity, not a probability**",
    "**InfoNCE**",
    "no dispersion estimate",
    "**every vector changes**",
    "reranking (see the sibling Qwen3 reranker pipeline), text generation or chat, classification heads",
...
EXTERNAL_WEIGHT_BYTES: dict[int, str] = {
    239_961: "banking_data/test.csv at PolyAI-LDN/task-specific-datasets@57ec275 (dataset)",
    839_073: "banking_data/train.csv at PolyAI-LDN/task-specific-datasets@57ec275 (dataset)",
}
EXTERNAL_WEIGHT_DIGESTS: dict[str, str] = {
    "b06e26ac675513959a63135f11b94ea7786ed02da65db93a5650d8838cbc664b": "banking_data/train.csv at PolyAI-LDN/task-specific-datasets@57ec275 (dataset)",
    "d12d6e3bc4c3103966ae786dc435913c0c563dfa328f5a3646d0e62cfeeb474d": "banking_data/test.csv at PolyAI-LDN/task-specific-datasets@57ec275 (dataset)",
}
