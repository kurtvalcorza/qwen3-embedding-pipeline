...
def test_no_validation_keeps_the_final_epoch_and_reloads_it(pipe, tmp_path):
    ...
    result = pipe.adapt(
        RECORDS[:8], None, instruction=INSTRUCTION, epochs=3, trainable_layers=1, batch_size=4
    )
...
def test_load_artifact_refuses_a_tensor_set_that_differs_from_the_recorded_configuration(pipe, tmp_path):
    ...
    pipe.adapt(RECORDS[:8], None, instruction=INSTRUCTION, epochs=1, trainable_layers=1, batch_size=4)
    ...
    with pytest.raises(ValueError, match="does not match its recorded configuration"):
        Qwen3EmbeddingPipeline.from_artifact(fewer, device="cpu")
    ...
    with pytest.raises(ValueError, match="tensor names differ"):
        Qwen3EmbeddingPipeline.from_artifact(extra, device="cpu")
    ...
    with pytest.raises(ValueError, match="does not match its recorded configuration"):
        Qwen3EmbeddingPipeline.from_artifact(other_layers, device="cpu")
