...
from qwen3_embedding_pipeline.pipeline import MANIFEST_NAME, MODEL_ID, MODEL_REVISION, Qwen3EmbeddingPipeline
...
def test_from_pretrained_refuses_without_snapshot_before_model_imports(tmp_path, forbid_model_imports):
    with pytest.raises(FileNotFoundError, match="allow_download=False"):
        Qwen3EmbeddingPipeline.from_pretrained(device="cpu", weights_dir=tmp_path, allow_download=False)
